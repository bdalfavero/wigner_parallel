...
num_points = range(int(args.min_samples), int(args.max_samples), interval)


for i in range(len(num_points)):

	f = h5py.File("domain_{}.h5".format(num_points[i]), 'w')

	num_points_dset = f.create_dataset("num_points", (1,), dtype='i')
	num_points_dset[0] = num_points[i]

	num_samples_dset = f.create_dataset("num_samples", (1,), dtype='i')
	num_samples_dset[0] = 50000
